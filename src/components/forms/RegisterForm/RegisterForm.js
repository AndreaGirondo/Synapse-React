  ...
  function RegisterForm({ isModalOpen, setIsModalOpen, setIsAuthenticated }) {
    ...
    const handleClick = async () => {
      ...
      try {
        ...
        if (response.errors) {
          ...
          localStorage.setItem('pseudo', response.user.name); 
    ...
    return (
      <div className=''>
        <button onClick={() => setIsModalOpen(true)} className='text-white font-poppins font-bold grid gad-8'>Inscription</button>
          <Modal
            isOpen={isModalOpen}
